...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-custom:executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "superpowers-custom:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip superpowers-custom: prefix if present
    const forceSuperpowers = skillName.startsWith('superpowers-custom:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^superpowers-custom:/, '') : skillName;

    // Try personal skills first (unless explicitly superpowers-custom:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superpowers-custom:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superpowers-custom:';
...
**Skills naming:**
- Superpowers skills: \`superpowers-custom:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'superpowers-custom:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "superpowers-custom:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill superpowers-custom:brainstorming | head -20`
