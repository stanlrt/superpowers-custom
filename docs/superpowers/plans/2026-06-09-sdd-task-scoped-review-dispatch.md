...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-custom:subagent-driven-development (recommended) or superpowers-custom:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
```
- **superpowers-custom:requesting-code-review** - Code review template for reviewer subagents
```
...
formatting functions. Use the superpowers-custom:subagent-driven-development skill
...
- A `Skill` invocation naming `superpowers-custom:subagent-driven-development`
...
post() {
    skill-called superpowers-custom:subagent-driven-development
    tool-called Agent
